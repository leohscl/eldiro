use crate::{env::Env, statement::Statement, utils::{extract_whitespace, tag}, val::Val};
...
impl Block {
    ...
    pub fn eval(&self, env: &Env) -> Result<Val, String> {
        ...
        self.exprs.iter().try_fold(Val::Empty, |_, statement| {
            statement.eval(&mut block_env)
        })
    }
}
...
mod tests {
        ...
        assert_eq!(
            Block {
                exprs: vec![
                    ...
                    Statement::Expr(
                        Expr::BindingUsage(BindingUsage{name: "a".to_string()})
                    ),
                ]
            }.eval(&Env::new()),
            Ok(Val::Number(10))
                ...
                (
                    ...
                    Block {
                        exprs: vec![
                            ...
                            Statement::Expr(
                                Expr::BindingUsage(BindingUsage{name: "b".to_string()})
                            ),
                        ]
                    }
                )
    ...
    fn eval_block_try_use_block_defined_var() {
        let env = Env::new();
        Block{exprs: vec![
            Statement::BindingDef(BindingDef{name: "one".to_string(), val: Expr::Number(Number(1))})
        ]}.eval(&env).unwrap();
        assert!(
            Block{exprs: vec![
                Statement::Expr(Expr::BindingUsage(BindingUsage{name: "one".to_string()}))
            ]}.eval(&env).is_err()
        )
    }
