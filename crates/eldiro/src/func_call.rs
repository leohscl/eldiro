use crate::{expr::Expr, utils::{extract_iden, extract_whitespace_separated1, take_while}, Env, Val};
...
impl FuncCall {
    ...
    pub(crate) fn eval(&self, env: &Env) -> Result<Val, String> {
        ...
        if callee_args.len() != self.args.len() {
            return Err(format!("Number of argument does not match function definition: \n {}: {:?}, \nfound {:?}", &self.name, self.args, callee_args));
        }
        // Create env with translated args
        let input_values: Vec<Val> = self.args.iter().map(|arg| arg.eval(&env)).collect::<Result<Vec<Val>, String>>()?;
        let mut function_env = Env::new();
        input_values.into_iter().zip(callee_args).for_each(|(value, arg_name)| {
            function_env.insert_binding(arg_name, value)
        });
...
#[cfg(test)]
mod tests {
    use crate::{expr::{BindingUsage, Expr, Number}, func_def::FuncDef, statement::Statement, Env};
    ...
    fn eval_call_unknown_args() {
        ...
        assert!(
            FuncCall {
                name: "identity".to_string(),
                args: vec![Expr::BindingUsage(BindingUsage {name: "x".to_string()})],
            }.eval(&env).is_err()
        );
    }
    ...
    #[test]
    fn eval_call_wrong_number_args() {
        let mut env = Env::new();
        env.insert_function(FuncDef {
